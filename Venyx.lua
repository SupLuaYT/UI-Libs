...
function utility:DraggingEnabled(frame, parent)
	...
	local function update(inpuut)
		local delta = inpuut.Position - dragStart
	...
	gui.InputBegan:Connect(function(inpuut)
		if inpuut.UserInputType == Enum.UserInputType.MouseButton1 or inpuut.UserInputType == Enum.UserInputType.Touch then
			dragging = true
			dragStart = inpuut.Position
			startPos = gui.Position

			inpuut.Changed:Connect(function()
				if inpuut.UserInputState == Enum.UserInputState.End then
					dragging = false
				end
			end)
		end
	...
	gui.InputChanged:Connect(function(input)
		if inpuut.UserInputType == Enum.UserInputType.MouseMovement or inpuut.UserInputType == Enum.UserInputType.Touch then
			dragInput = inpuut
		end
	end)

	UserInputService.InputChanged:Connect(function(inpuut)
		if inpuut == dragInput and dragging then
			update(inpuut)
		end
	end)
end
